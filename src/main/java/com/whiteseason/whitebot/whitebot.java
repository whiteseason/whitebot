package com.whiteseason.whitebot;



import com.whiteseason.whitebot.Service.AuthorityUserC;
import com.whiteseason.whitebot.Service.UpdataUser;
import com.whiteseason.whitebot.bean.User;
import com.whiteseason.whitebot.client.Client;
import com.whiteseason.whitebot.client.msgHandle;
 ...
 * 具体功能可以查看文档
 */
public class whitebot extends JcqAppAbstract implements ICQVer, IMsg, IRequest {


    private static HashMap<String,String> whitebook = new HashMap();
    private static HashMap<String,String> authorityUser = new HashMap();
    AuthorityUserC ac;
    public whitebot (CoolQ CQ){
        super(CQ);
    }
    ...
    public static void main(String[] args) {
        ...
        CQDebug CQ = new CQDebug();//new CQDebug("应用目录","应用名称") 可以用此构造器初始化应用的目录
        CQ.logInfo("[JCQ] TEST Demo", "测试启动");// 现在就可以用CQ变量来执行任何想要的操作了
        // 要测试主类就先实例化一个主类对象
        whitebot demo = new whitebot(CQ);
        ...
        demo.privateMsg(0, 10001, 447503971L, "!setname Kotoki 1 3 3 7", 0);
        // 模拟群聊消息
        // 开始模拟群聊消息
//        demo.groupMsg(0, 10009, 427984429L, 3333333334L, "", "[CQ:at,qq=2222222224] 来一起玩游戏，开车开车", 0);
    ...
    public String appInfo() {
        ...
        String AppID = "com.whiteseason.whitebot";// 记住编译后的文件和json也要使用appid做文件名
        /*
    ...
    public int startup() {
        ...
        ac = new AuthorityUserC(appDirectory);
        return 0;
    }
    ...
    public int enable() {
        enable = true;
        whitebook = ac.getWhitebook();
        CQ.logInfo("[whitebot]whitebookInfo:","获取白名单成员共"+whitebook.size());
        if (UpdataUser.controlled()){
            CQ.logInfo("[whitebot]Time Controlled","启动成功");
            UpdataUser.setListener(new UpdataUser.OnUpdateListener() {
                @Override
                public void getList(List<List<User>> list) {
                    List<User> unbindUser = list.get(0);
                    String string = "";
                    int flag = 0;
                    for (User u :
                            unbindUser) {
                        if(whitebook.get(u.getQq()) == null) {
                            string += u.getQq() + "\n";
//                            CQ.sendGroupMsg(885984366, "[CQ:at,qq=" + u.getQq() + "]您的pp或bp超限，即将被踢出本群，请及时加入osu！新人进阶群（群号：758120648）");
                            flag++;
                        }
                    }
                    CQ.logInfoSuccess("[whitebot]Time Controlled","超限名单查询成功");
                    if(flag != 0)
                        CQ.sendGroupMsg(695600319,"超限名单更新：\n"+string);
    ...
    public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
        // 这里处理消息
        CQ.logInfoSuccess("[whitebot]privitemsg","获取信息成功");
        String handlemsg = new Client().publicHandle(msg,fromQQ);
        CQ.logInfoSuccess("[whitebot]privitemsg",handlemsg);
        if(handlemsg != null){
            CQ.sendPrivateMsg(fromQQ,handlemsg);
        }
        return MSG_IGNORE;
    }
    ...
    public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg,
        ...
        // 这里处理消息
        if(authorityUser.get(fromQQ) != null){
            if(msg.indexOf("设置管理员") == 0){
                String msg1[] = msg.split(":");
                if(msg.length() == 2 || CC.getAt(msg) != -1 || CC.getAt(msg) != -1000){
                    whitebook.put(Double.toString(CC.getAt(msg)),msg1[1]);
                    CQ.sendGroupMsg(fromGroup,"添加管理成功");
                }
            }
        }
        String handlemsg = new Client().publicHandle(msg,fromQQ);
        if(handlemsg != null){
            CQ.sendGroupMsg(fromGroup,handlemsg);
        }
        return MSG_IGNORE;
    }
    ...
    public int groupMemberIncrease(int subtype, int sendTime, long fromGroup, long fromQQ, long beingOperateQQ) {
        // 这里处理消息
        CQ.sendGroupMsg(fromGroup,new msgHandle().joinGroup(beingOperateQQ,fromGroup));
        return MSG_IGNORE;
    }
